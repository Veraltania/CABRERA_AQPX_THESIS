...
# --- IMPORTS ---
# CHANGED: Import the new OOP classes
try:
...
# CHANGED: Map string names directly to the classes
ALGO_MAP = {'DE': DEOptimizer, 'GA': GAOptimizer, 'PSO': PSOOptimizer}


# --- 1. WORKER FUNCTION ---
def worker(task):
    # CHANGED: Unpack the separated configs and base directory
    algo_name, pop_size, base_config, tf_params, base_output_folder = task
    ...
    try:
        # CHANGED: Instantiate the correct Optimizer class and run it
...
if __name__ == "__main__":
    ...
    # CHANGED: Explicit base output directory
    BASE_OUTPUT_DIR = "results_population_sweep"

    # CHANGED: Separated algorithm config from Transfer Function config
    shared_config = {
        "patience_limit": 25,
        "max_iters": 200,
        "improvement_tol": 1.0,
    ...
    # CHANGED: Pass BOTH shared_config and tf_params, and BASE_OUTPUT_DIR into the task tuple
    tasks = [(algo, size, shared_config, tf_params, BASE_OUTPUT_DIR)
             for algo in ALGO_MAP.keys()
             for size in pop_sizes]

    num_cores = multiprocessing.cpu_count() - 1 or 1  # Leaves one core free
